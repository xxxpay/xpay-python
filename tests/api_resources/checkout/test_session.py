...
class TestSession(object):
    def test_is_creatable(self, request_mock):
        resource = xpay.checkout.Session.create(
        ...
        assert isinstance(resource, xpay.checkout.Session)

    def test_is_retrievable(self, request_mock):
        resource = xpay.checkout.Session.retrieve(TEST_RESOURCE_ID)
...
class TestSessionLineItems(object):
    def test_is_listable(self, request_mock):
        resources = xpay.checkout.Session.list_line_items(TEST_RESOURCE_ID)
        request_mock.assert_requested(
