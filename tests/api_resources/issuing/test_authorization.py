...
class TestAuthorization(object):
    def test_is_listable(self, request_mock):
        resources = xpay.issuing.Authorization.list()
        request_mock.assert_requested("get", "/v1/issuing/authorizations")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], xpay.issuing.Authorization)

    def test_is_modifiable(self, request_mock):
        resource = xpay.issuing.Authorization.modify(
            TEST_RESOURCE_ID, metadata={"key": "value"}
        )
        request_mock.assert_requested(
            "post", "/v1/issuing/authorizations/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, xpay.issuing.Authorization)

    def test_is_retrievable(self, request_mock):
        resource = xpay.issuing.Authorization.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
    ...
    def test_can_approve_classmethod(self, request_mock):
        resource = xpay.issuing.Authorization.approve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post", "/v1/issuing/authorizations/%s/approve" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, xpay.issuing.Authorization)
    ...
    def test_can_decline_classmethod(self, request_mock):
        resource = xpay.issuing.Authorization.decline(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post", "/v1/issuing/authorizations/%s/decline" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, xpay.issuing.Authorization)
