...
class TestWebhookEndpoint(object):
    def test_is_listable(self, request_mock):
        resources = xpay.WebhookEndpoint.list()
        request_mock.assert_requested("get", "/v1/webhook_endpoints")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], xpay.WebhookEndpoint)

    def test_is_retrievable(self, request_mock):
        resource = xpay.WebhookEndpoint.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "get", "/v1/webhook_endpoints/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, xpay.WebhookEndpoint)

    def test_is_creatable(self, request_mock):
        resource = xpay.WebhookEndpoint.create(
    ...
    def test_is_modifiable(self, request_mock):
        resource = xpay.WebhookEndpoint.modify(
            TEST_RESOURCE_ID,
    ...
    def test_can_delete(self, request_mock):
        resource = xpay.WebhookEndpoint.delete(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "delete", "/v1/webhook_endpoints/%s" % TEST_RESOURCE_ID
        )
        assert resource.deleted is True
