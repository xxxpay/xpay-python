...
class TestInvoiceItem(object):
    def test_is_listable(self, request_mock):
        resources = xpay.InvoiceItem.list()
        request_mock.assert_requested("get", "/v1/invoiceitems")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], xpay.InvoiceItem)

    def test_is_retrievable(self, request_mock):
        resource = xpay.InvoiceItem.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "get", "/v1/invoiceitems/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, xpay.InvoiceItem)

    def test_is_creatable(self, request_mock):
        resource = xpay.InvoiceItem.create(
    ...
    def test_can_delete(self, request_mock):
        resource = xpay.InvoiceItem.delete(TEST_RESOURCE_ID)
        request_mock.assert_requested(
    ...
    def test_is_modifiable(self, request_mock):
        resource = xpay.InvoiceItem.modify(
