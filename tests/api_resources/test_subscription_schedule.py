...
class TestSubscriptionScheduleSchedule(object):
    def test_is_listable(self, request_mock):
        resources = xpay.SubscriptionSchedule.list()
        request_mock.assert_requested("get", "/v1/subscription_schedules")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], xpay.SubscriptionSchedule)

    def test_is_retrievable(self, request_mock):
        resource = xpay.SubscriptionSchedule.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "get", "/v1/subscription_schedules/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, xpay.SubscriptionSchedule)

    def test_is_creatable(self, request_mock):
        resource = xpay.SubscriptionSchedule.create(customer="cus_123")
    ...
    def test_is_modifiable(self, request_mock):
        resource = xpay.SubscriptionSchedule.modify(
    ...
    def test_can_cancel_classmethod(self, request_mock):
        resource = xpay.SubscriptionSchedule.cancel(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post", "/v1/subscription_schedules/%s/cancel" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, xpay.SubscriptionSchedule)
    ...
    def test_can_release_classmethod(self, request_mock):
        resource = xpay.SubscriptionSchedule.release(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post", "/v1/subscription_schedules/%s/release" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, xpay.SubscriptionSchedule)
