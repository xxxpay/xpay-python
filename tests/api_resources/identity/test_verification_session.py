...
class TestVerificationSession(object):
    def test_is_creatable(self, request_mock):
        resource = xpay.identity.VerificationSession.create(type="id_number")
        request_mock.assert_requested(
            "post", "/v1/identity/verification_sessions"
        )
        assert isinstance(resource, xpay.identity.VerificationSession)

    def test_is_listable(self, request_mock):
        resources = xpay.identity.VerificationSession.list()
        request_mock.assert_requested(
            "get", "/v1/identity/verification_sessions"
        )
        assert isinstance(resources.data, list)
        assert isinstance(
            resources.data[0], xpay.identity.VerificationSession
        )

    def test_is_modifiable(self, request_mock):
        resource = xpay.identity.VerificationSession.modify(
    ...
    def test_is_retrievable(self, request_mock):
        resource = xpay.identity.VerificationSession.retrieve(
            TEST_RESOURCE_ID
        )
        request_mock.assert_requested(
    ...
    def test_can_cancel_classmethod(self, request_mock):
        resource = xpay.identity.VerificationSession.cancel(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post",
            "/v1/identity/verification_sessions/%s/cancel" % TEST_RESOURCE_ID,
        )
    ...
    def test_can_redact_classmethod(self, request_mock):
        resource = xpay.identity.VerificationSession.redact(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post",
            "/v1/identity/verification_sessions/%s/redact" % TEST_RESOURCE_ID,
        )
