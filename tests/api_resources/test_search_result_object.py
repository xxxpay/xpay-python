...
class TestSearchResultObject(object):
    ...
    def search_result_object(self):
        return xpay.SearchResultObject.construct_from(
    ...
    def test_search(self, request_mock, search_result_object):
        ...
        res = search_result_object.search(
            myparam="you", xpay_account="acct_123"
        )

        request_mock.assert_requested(
            "get", "/my/path", {"myparam": "you"}, None
        )
        assert isinstance(res, xpay.SearchResultObject)
    ...
    def test_is_empty(self):
        sro = xpay.SearchResultObject.construct_from({"data": []}, None)
        assert sro.is_empty is True

    def test_empty_search_result(self):
        sro = xpay.SearchResultObject.empty_search_result()
        assert sro.is_empty

    def test_iter(self):
        arr = [{"id": 1}, {"id": 2}, {"id": 3}]
        expected = xpay.util.convert_to_xpay_object(arr)
        sro = xpay.SearchResultObject.construct_from({"data": arr}, None)
    ...
    def test_next_search_result_page_empty_search_result(self):
        ...
        next_sro = sro.next_search_result_page()
        assert next_sro == xpay.SearchResultObject.empty_search_result()
    ...
    def test_serialize_empty_search_result(self):
        ...
        deserialized = xpay.SearchResultObject.construct_from(
