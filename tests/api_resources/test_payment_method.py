...
class TestPaymentMethod(object):
    def test_is_listable(self, request_mock):
        resources = xpay.PaymentMethod.list(customer="cus_123", type="card")
        request_mock.assert_requested("get", "/v1/payment_methods")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], xpay.PaymentMethod)

    def test_is_retrievable(self, request_mock):
        resource = xpay.PaymentMethod.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "get", "/v1/payment_methods/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, xpay.PaymentMethod)

    def test_is_creatable(self, request_mock):
        resource = xpay.PaymentMethod.create(
    ...
    def test_is_modifiable(self, request_mock):
        resource = xpay.PaymentMethod.modify(
    ...
    def test_can_attach_classmethod(self, request_mock):
        resource = xpay.PaymentMethod.attach(
    ...
    def test_can_detach_classmethod(self, request_mock):
        resource = xpay.PaymentMethod.detach(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post", "/v1/payment_methods/%s/detach" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, xpay.PaymentMethod)
