...
class TestAPIResource(object):
    class MyResource(xpay.api_resources.abstract.APIResource):
    ...
    def test_convert_to_xpay_object(self):
        sample = {
            "foo": "bar",
        ...
        converted = xpay.util.convert_to_xpay_object(
            sample, "akey", None, None
        )

        # Types
        assert isinstance(converted, xpay.xpay_object.XPayObject)
        assert isinstance(converted.adict, xpay.Charge)
        assert len(converted.alist) == 1
        assert isinstance(converted.alist[0], xpay.Customer)
