...
class TestCreditNote(object):
    def test_is_listable(self, request_mock):
        resources = xpay.CreditNote.list()
        request_mock.assert_requested("get", "/v1/credit_notes")
        assert isinstance(resources.data, list)
        assert isinstance(resources.data[0], xpay.CreditNote)

    def test_is_retrievable(self, request_mock):
        resource = xpay.CreditNote.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "get", "/v1/credit_notes/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, xpay.CreditNote)

    def test_is_creatable(self, request_mock):
        resource = xpay.CreditNote.create(
    ...
    def test_is_modifiable(self, request_mock):
        resource = xpay.CreditNote.modify(
    ...
    def test_can_preview(self, request_mock):
        resource = xpay.CreditNote.preview(invoice="in_123", amount=500)
    ...
    def test_can_void_credit_note_classmethod(self, request_mock):
        resource = xpay.CreditNote.void_credit_note(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "post", "/v1/credit_notes/%s/void" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, xpay.CreditNote)
