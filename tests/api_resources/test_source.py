...
class TestSource(object):
    def test_is_retrievable(self, request_mock):
        resource = xpay.Source.retrieve(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "get", "/v1/sources/%s" % TEST_RESOURCE_ID
        )
        assert isinstance(resource, xpay.Source)

    def test_is_creatable(self, request_mock):
        resource = xpay.Source.create(type="card", token="tok_123")
    ...
    def test_is_detachable_when_attached(self, request_mock):
        resource = xpay.Source.construct_from(
            {
                "id": TEST_RESOURCE_ID,
                "object": "source",
                "customer": "cus_123",
...
class TestSourceTransactions(object):
    def test_is_listable(self, request_mock):
        resource = xpay.Source.list_source_transactions(TEST_RESOURCE_ID)
        request_mock.assert_requested(
            "get", "/v1/sources/%s/source_transactions" % TEST_RESOURCE_ID
        )
        assert isinstance(resource.data, list)
        assert isinstance(resource.data[0], xpay.SourceTransaction)
