...
class TestListObject(object):
    ...
    def list_object(self):
        return xpay.ListObject.construct_from(
    ...
    def test_list(self, request_mock, list_object):
        ...
        res = list_object.list(myparam="you", xpay_account="acct_123")

        request_mock.assert_requested(
            "get", "/my/path", {"myparam": "you"}, None
        )
        assert isinstance(res, xpay.ListObject)
        assert res.xpay_account == "acct_123"
        assert isinstance(res.data, list)
        assert isinstance(res.data[0], xpay.Charge)
    ...
    def test_create(self, request_mock, list_object):
        ...
        res = list_object.create(myparam="eter", xpay_account="acct_123")

        request_mock.assert_requested(
            "post", "/my/path", {"myparam": "eter"}, None
        )
        assert isinstance(res, xpay.Charge)
        assert res.foo == "bar"
        assert res.xpay_account == "acct_123"
    ...
    def test_create_maintains_list_properties(self, request_mock, list_object):
        ...
        # test otherwise
        charge = xpay.Charge.retrieve("ch_123", api_key="sk_test_custom")
    ...
    def test_retrieve(self, request_mock, list_object):
        ...
        res = list_object.retrieve(
            "myid", myparam="cow", xpay_account="acct_123"
        )
    ...
    def test_is_empty(self):
        lo = xpay.ListObject.construct_from({"data": []}, None)
        assert lo.is_empty is True

    def test_empty_list(self):
        lo = xpay.ListObject.empty_list()
        assert lo.is_empty

    def test_iter(self):
        arr = [{"id": 1}, {"id": 2}, {"id": 3}]
        expected = xpay.util.convert_to_xpay_object(arr)
        lo = xpay.ListObject.construct_from({"data": arr}, None)
        assert list(lo) == expected

    def test_iter_reversed(self):
        arr = [{"id": 1}, {"id": 2}, {"id": 3}]
        expected = xpay.util.convert_to_xpay_object(list(reversed(arr)))
        lo = xpay.ListObject.construct_from({"data": arr}, None)
    ...
    def test_next_page_empty_list(self):
        ...
        next_lo = lo.next_page()
        assert next_lo == xpay.ListObject.empty_list()
    ...
    def test_serialize_empty_list(self):
        ...
        deserialized = xpay.ListObject.construct_from(
    ...
    def test_serialize_nested_empty_list(self):
        ...
        serialized = str(obj)
        deserialized = xpay.xpay_object.XPayObject.construct_from(
            json.loads(serialized), "mykey"
        )
...
class TestAutoPaging:
    ...
    def test_class_method_two_pages(self, request_mock):
        ...
        seen = [
            item["id"]
            for item in xpay.Charge.auto_paging_iter(limit=25, foo="bar")
        ]
