...
class TestXPayError(object):
    ...
    def test_error_object_not_dict(self):
        err = error.XPayError("message", json_body={"error": "not a dict"})
        assert err.error is None


class TestXPayErrorWithParamCode(object):
    def test_repr(self):
        err = error.CardError(
            "öre",
            param="cparam",
