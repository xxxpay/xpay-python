...
class XPayClientTestCase(object):
    REQUEST_LIBRARIES = [
        ("urlfetch", "xpay.http_client.urlfetch"),
        ("requests", "xpay.http_client.requests"),
        ("pycurl", "xpay.http_client.pycurl"),
        ("urllib.request", "xpay.http_client.urllibrequest"),
    ]
...
class TestNewDefaultHttpClient(XPayClientTestCase):
    @pytest.fixture(autouse=True)
    def setup_warnings(self, request_mocks):
        original_filters = xpay.http_client.warnings.filters[:]
        xpay.http_client.warnings.simplefilter("ignore")
        yield
        xpay.http_client.warnings.filters = original_filters

    def check_default(self, none_libs, expected):
        for lib in none_libs:
            setattr(xpay.http_client, lib, None)

        inst = xpay.http_client.new_default_http_client()

        assert isinstance(inst, expected)

    def test_new_default_http_client_urlfetch(self):
        self.check_default((), xpay.http_client.UrlFetchClient)

    def test_new_default_http_client_requests(self):
        self.check_default(("urlfetch",), xpay.http_client.RequestsClient)

    def test_new_default_http_client_pycurl(self):
        self.check_default(
            ("urlfetch", "requests"), xpay.http_client.PycurlClient
        )
...
class TestRetrySleepTimeDefaultHttpClient(XPayClientTestCase):
    ...
    def mock_max_delay(self, new_value):
        original_value = xpay.http_client.HTTPClient.MAX_DELAY
        xpay.http_client.HTTPClient.MAX_DELAY = new_value
        try:
            yield self
        finally:
            xpay.http_client.HTTPClient.MAX_DELAY = original_value

    def test_sleep_time_exponential_back_off(self):
        client = xpay.http_client.new_default_http_client()
    ...
    def test_initial_delay_as_minimum(self):
        ...
        initial_delay = xpay.http_client.HTTPClient.INITIAL_DELAY
        self.assert_sleep_times(client, [initial_delay] * 5)

    def test_maximum_delay(self):
        client = xpay.http_client.new_default_http_client()
        client._add_jitter_time = lambda t: t
        max_delay = xpay.http_client.HTTPClient.MAX_DELAY
    ...
    def test_randomness_added(self):
        ...
        base_value = xpay.http_client.HTTPClient.INITIAL_DELAY * random_value

        with self.mock_max_delay(10):
            expected = [
                xpay.http_client.HTTPClient.INITIAL_DELAY,
                base_value * 2,
                base_value * 4,
                base_value * 8,
                base_value * 16,
...
class TestRetryConditionsDefaultHttpClient(XPayClientTestCase):
    ...
    def test_should_retry_on_xpay_should_retry_true(self, mocker):
        client = xpay.http_client.new_default_http_client()
        client._max_network_retries = lambda: 1
        headers = {"xpay-should-retry": "true"}
    ...
    def test_should_retry_on_xpay_should_retry_false(self, mocker):
        client = xpay.http_client.new_default_http_client()
        client._max_network_retries = lambda: 1
        headers = {"xpay-should-retry": "false"}
...
class TestHTTPClient(object):
    @pytest.fixture(autouse=True)
    def setup_xpay(self):
        orig_attrs = {"enable_telemetry": xpay.enable_telemetry}
    ...
    def test_sends_telemetry_on_second_request(self, mocker):
        class TestClient(xpay.http_client.HTTPClient):
            pass
        ...
        assert "X-XPay-Client-Telemetry" in args[2]

        telemetry = json.loads(args[2]["X-XPay-Client-Telemetry"])
...
class ClientTestBase(object):
    ...
    def test_exception(self, request_mock, mock_error):
        ...
        with pytest.raises(xpay.error.APIConnectionError):
...
class TestRequestsClient(XPayClientTestCase, ClientTestBase):
    REQUEST_CLIENT = xpay.http_client.RequestsClient
...
class TestRequestClientRetryBehavior(TestRequestsClient):
    ...
    def connection_error(self, session):
        ...
        def connection_error(given_exception):
            with pytest.raises(xpay.error.APIConnectionError) as error:
    ...
    def test_handle_request_error_should_not_retry(
        ...
        assert "not verify XPay's SSL certificate" in error.user_message
...
class TestUrlFetchClient(XPayClientTestCase, ClientTestBase):
    REQUEST_CLIENT = xpay.http_client.UrlFetchClient
...
class TestUrllib2Client(XPayClientTestCase, ClientTestBase):
    REQUEST_CLIENT = xpay.http_client.Urllib2Client
...
class TestPycurlClient(XPayClientTestCase, ClientTestBase):
    REQUEST_CLIENT = xpay.http_client.PycurlClient
    ...
    def bio_mock(self, mocker):
        bio_patcher = mocker.patch("xpay.util.io.BytesIO")
    ...
    def mock_error(self):
        def mock_error(mock):
            ...
            xpay.http_client.pycurl.error = FakeException
            mock.perform.side_effect = xpay.http_client.pycurl.error
...
class TestAPIEncode(XPayClientTestCase):
    def test_encode_dict(self):
        body = {"foo": {"dob": {"month": 1}, "name": "bat"}}

        values = [t for t in xpay.api_requestor._api_encode(body)]
