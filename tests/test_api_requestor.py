...
from xpay.xpay_response import XPayResponse, XPayStreamResponse
...
class APIHeaderMatcher(object):
    ...
    def __init__(
        ...
        self.api_key = api_key or xpay.api_key
        self.extra = extra
    ...
    def __eq__(self, other):
        return (
            ...
            and self._x_xpay_ua_contains_app_info(other)
            and self._x_xpay_ua_handles_failed_platform_function(other)
    ...
    def _x_xpay_ua_contains_app_info(self, other):
        if self.app_info:
            ua = json.loads(other["X-XPay-Client-User-Agent"])
    ...
    def _x_xpay_ua_handles_failed_platform_function(self, other):
...
class QueryMatcher(object):
    ...
    def __eq__(self, other):
        ...
        parsed = xpay.util.parse_qsl(query)
...
class UrlMatcher(object):
    ...
    def __eq__(self, other):
        ...
        q_matcher = QueryMatcher(xpay.util.parse_qsl(self.exp_parts.query))
...
class TestAPIRequestor(object):
    ...
    def http_client(self, mocker):
        http_client = mocker.Mock(xpay.http_client.HTTPClient)
    ...
    def requestor(self, http_client):
        requestor = xpay.api_requestor.APIRequestor(client=http_client)
        return requestor
    ...
    def check_call(self, http_client):
        def check_call(
            ...
            if not abs_url:
                abs_url = "%s%s" % (xpay.api_base, self.valid_path)
            if not headers:
    ...
    def encoder_check(self, key):
        ...
        stk = []
        fn = getattr(xpay.api_requestor.APIRequestor, "encode_%s" % (key,))
    ...
    def _test_encode_naive_datetime(self):
        ...
        xpay.api_requestor.APIRequestor.encode_datetime(
    ...
    def test_dictionary_list_encoding(self):
        ...
        encoded = list(xpay.api_requestor._api_encode(params))
        key, value = encoded[0]
    ...
    def test_url_construction(self, requestor, mock_response, check_call):
        CASES = (
            ("%s?foo=bar" % xpay.api_base, "", {"foo": "bar"}),
            ("%s?foo=bar" % xpay.api_base, "?", {"foo": "bar"}),
            (xpay.api_base, "", {}),
            (
                "%s/%%20spaced?foo=bar%%24&baz=5" % xpay.api_base,
                "/%20spaced?foo=bar%24",
                {"baz": "5"},
            ),
            (
                "%s?foo=bar&foo=bar" % xpay.api_base,
                "?foo=bar",
                {"foo": "bar"},
            ),
        )
    ...
    def test_empty_methods(self, requestor, mock_response, check_call):
        for meth in VALID_API_METHODS:
            ...
            assert isinstance(resp, XPayResponse)
    ...
    def test_empty_methods_streaming_response(
        ...
        for meth in VALID_API_METHODS:
            ...
            assert isinstance(resp, XPayStreamResponse)
    ...
    def test_methods_with_params_and_response(
        ...
        for method in VALID_API_METHODS:
            ...
            if method == "post":
                check_call(
                    method,
                    post_data=QueryMatcher(xpay.util.parse_qsl(encoded)),
                )
            else:
                abs_url = "%s%s?%s" % (
                    xpay.api_base,
    ...
    def test_uses_instance_key(self, http_client, mock_response, check_call):
        key = "fookey"
        requestor = xpay.api_requestor.APIRequestor(key, client=http_client)
    ...
    def test_uses_instance_api_version(
        ...
        requestor = xpay.api_requestor.APIRequestor(
        ...
        check_call(
            ...
            headers=APIHeaderMatcher(
                extra={"XPay-Version": "fooversion"}, request_method="get"
            ),
        )
    ...
    def test_uses_instance_account(
        ...
        check_call(
            "get",
            headers=APIHeaderMatcher(
                extra={"XPay-Account": account}, request_method="get"
            ),
        )

    def test_sets_default_http_client(self, http_client):
        assert not xpay.default_http_client

        xpay.api_requestor.APIRequestor(client=http_client)

        # default_http_client is not populated if a client is provided
        assert not xpay.default_http_client

        xpay.api_requestor.APIRequestor()

        # default_http_client is set when no client is specified
        assert xpay.default_http_client

        new_default_client = xpay.default_http_client
        xpay.api_requestor.APIRequestor()

        # the newly created client is reused
        assert xpay.default_http_client == new_default_client
    ...
    def test_uses_app_info(self, requestor, mock_response, check_call):
        try:
            ...
            ua = "XPay/v1 PythonBindings/%s" % (xpay.version.VERSION,)
    ...
    def test_fails_without_api_key(self, requestor):
        ...
        with pytest.raises(xpay.error.AuthenticationError):
            requestor.request("get", self.valid_path, {})

    def test_invalid_request_error_404(self, requestor, mock_response):
        mock_response('{"error": {}}', 404)

        with pytest.raises(xpay.error.InvalidRequestError):
            requestor.request("get", self.valid_path, {})
    ...
    def test_idempotency_error(self, requestor, mock_response):
        ...
        with pytest.raises(xpay.error.IdempotencyError):
            requestor.request("get", self.valid_path, {})
    ...
    def test_rate_limit_error(self, requestor, mock_response):
        ...
        with pytest.raises(xpay.error.RateLimitError):
            requestor.request("get", self.valid_path, {})
    ...
    def test_server_error(self, requestor, mock_response):
        ...
        with pytest.raises(xpay.error.APIError):
            requestor.request("get", self.valid_path, {})
