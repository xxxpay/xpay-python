...
class RequestMock(object):
    def __init__(self, mocker):
        ...
        self._real_request = xpay.api_requestor.APIRequestor.request
        self._real_request_stream = (
            xpay.api_requestor.APIRequestor.request_stream
        )
        self._stub_request_handler = StubRequestHandler()

        self.constructor_patcher = self._mocker.patch(
            "xpay.api_requestor.APIRequestor.__init__",
            side_effect=xpay.api_requestor.APIRequestor.__init__,
            autospec=True,
        )

        self.request_patcher = self._mocker.patch(
            "xpay.api_requestor.APIRequestor.request",
...
class StubRequestHandler(object):
    ...
    def get_response(self, method, url, expect_stream=False):
        if (method, url) in self._entries:
            ...
            if is_streaming:
                xpay_response = XPayStreamResponse(
                    util.io.BytesIO(str.encode(rbody)), rcode, rheaders
                )
            else:
                xpay_response = XPayResponse(rbody, rcode, rheaders)
            return xpay_response
