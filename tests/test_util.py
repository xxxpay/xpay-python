...
class TestUtil(object):
    ...
    def test_test_apikey(self, mocker):
        mocker.patch("xpay.api_key", "sk_test_KOWobxXidxNlIx")
    ...
    def test_live_apikey(self, mocker):
        mocker.patch("xpay.api_key", "sk_live_axwITqZSgTUXSN")
        link = util.dashboard_link(self.DUMMY_REQ_ID)
        assert (
            link
    ...
    def test_no_apikey(self, mocker):
        mocker.patch("xpay.api_key", None)
        link = util.dashboard_link(self.DUMMY_REQ_ID)
        assert (
            link
            == "https://dashboard.stripe.com/test/logs/" + self.DUMMY_REQ_ID
    ...
    def test_old_apikey(self, mocker):
        mocker.patch("xpay.api_key", "axwITqZSgTUXSN")
        link = util.dashboard_link(self.DUMMY_REQ_ID)
        assert (
            link
            == "https://dashboard.stripe.com/test/logs/" + self.DUMMY_REQ_ID
    ...
    def log_test_loop(self, test_cases, logging_func, logger_name, mocker):
        for case in test_cases:
            try:
                ...
                mocker.patch("xpay.log", case.flag)
                mocker.patch("xpay.util.STRIPE_LOG", case.env)
    ...
    def test_log_debug(self, mocker):
        # (STRIPE_LOG, xpay.log): should_output?
        test_cases = [
        ...
        self.log_test_loop(
            ...
            logger_name="xpay.util.logger.debug",
            mocker=mocker,
        )
    ...
    def test_log_info(self, mocker):
        ...
        self.log_test_loop(
            ...
            logger_name="xpay.util.logger.info",
            mocker=mocker,
        )
    ...
    def test_convert_to_xpay_object_and_back(self):
        resp = {
            "object": "balance",
            "available": [
                {
        ...
        obj = util.convert_to_xpay_object(resp)
        assert isinstance(obj, xpay.Balance)
        assert isinstance(obj.available, list)
        assert isinstance(obj.available[0], xpay.xpay_object.XPayObject)
