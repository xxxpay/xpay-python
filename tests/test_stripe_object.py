...
class TestXPayObject(object):
    def test_initializes_with_parameters(self):
        obj = xpay.xpay_object.XPayObject(
    ...
    def test_access(self):
        obj = xpay.xpay_object.XPayObject("myid", "mykey", myparam=5)
    ...
    def test_refresh_from(self, mocker):
        obj = xpay.xpay_object.XPayObject.construct_from(
        ...
        assert obj.foo == "bar"
        assert obj["trans"] == "me"
        assert obj.xpay_version is None
        assert obj.xpay_account is None
        ...
        obj.refresh_from(
            {"foo": "baz", "johnny": 5},
            "key2",
            xpay_version="2017-08-15",
            xpay_account="acct_foo",
        ...
        assert obj.xpay_version == "2017-08-15"
        assert obj.xpay_account == "acct_foo"
    ...
    def test_passing_nested_refresh(self):
        ...
        assert nested.xpay_account == "acct_foo"
    ...
    def test_refresh_from_nested_object(self):
        ...
        assert isinstance(obj.lines, xpay.ListObject)
        assert isinstance(obj.lines.data[0], xpay.InvoiceLineItem)
        assert isinstance(
            obj.lines.data[0].price, xpay.xpay_object.XPayObject
        )
        assert isinstance(obj.lines.data[0].price, xpay.Price)
    ...
    def test_repr(self):
        obj = xpay.xpay_object.XPayObject("foo", "bar", myparam=5)
        ...
        assert "<XPayObject \u4e00boo\u1f00" in res
    ...
    def test_deletion(self):
        obj = xpay.xpay_object.XPayObject("id", "key")
    ...
    def test_copy(self):
        nested = xpay.xpay_object.XPayObject.construct_from(
            {"value": "bar"}, "mykey"
        )
        obj = xpay.xpay_object.XPayObject.construct_from(
            {"empty": "", "value": "foo", "nested": nested},
            "mykey",
            xpay_account="myaccount",
        )
        ...
        assert copied.xpay_account == "myaccount"
    ...
    def test_to_dict_recursive(self):
        foo = xpay.xpay_object.XPayObject.construct_from(
            {"value": "foo"}, "mykey"
        )
        bar = xpay.xpay_object.XPayObject.construct_from(
            {"value": "bar"}, "mykey"
        )
        obj = xpay.xpay_object.XPayObject.construct_from(
        ...
        assert not isinstance(
            d["nested"][0], xpay.xpay_object.XPayObject
        )
        assert not isinstance(
            d["nested"][1], xpay.xpay_object.XPayObject
        )

    def test_serialize_empty_string_unsets(self):
        class SerializeToEmptyString(xpay.xpay_object.XPayObject):
