...
from xpay.request_metrics import RequestMetrics
...
try:
    ...
    try:
        ...
        if (major, minor, patch) < (0, 8, 8):
            sys.stderr.write(
                "Warning: the XPay library requires that your Python "
                '"requests" library be newer than version 0.8.8, but your '
                '"requests" library is version %s. XPay will fall back to '
                "an alternate HTTP library so everything should work. We "
                'recommend upgrading your "requests" library. If you have any '
                "questions, please contact support@xpay.com. (HINT: running "
...
def new_default_http_client(*args, **kwargs):
    if urlfetch:
        ...
        if sys.version_info < (2, 7, 9):
            warnings.warn(
                "Warning: the XPay library is falling back to urllib2 "
...
class HTTPClient(object):
    ...
    def _should_retry(self, response, api_connection_error, num_retries):
        ...
        if rheaders is not None and "xpay-should-retry" in rheaders:
            if rheaders["xpay-should-retry"] == "false":
                return False
            if rheaders["xpay-should-retry"] == "true":
                return True
        ...
        #
        # Note that we expect the xpay-should-retry header to be false
    ...
    def _max_network_retries(self):
        from xpay import max_network_retries
    ...
    def _add_telemetry_header(self, headers):
        ...
        if xpay.enable_telemetry and last_request_metrics:
            telemetry = {
    ...
    def _record_request_metrics(self, response, request_start):
        _, _, rheaders = response
        if "Request-Id" in rheaders and xpay.enable_telemetry:
...
class RequestsClient(HTTPClient):
    ...
    def _request_internal(self, method, url, headers, post_data, is_streaming):
        ...
        if self._verify_ssl_certs:
            kwargs["verify"] = xpay.ca_bundle_path
        else:
            kwargs["verify"] = False
...
class PycurlClient(HTTPClient):
    ...
    def _request_internal(self, method, url, headers, post_data, is_streaming):
        ...
        if self._verify_ssl_certs:
            self._curl.setopt(self.pycurl.CAINFO, xpay.ca_bundle_path)
        else:
