...
if TYPE_CHECKING:
    ...
    from xpay.api_resources.line_item import LineItem
    from xpay.api_resources.payment_intent import PaymentIntent
    from xpay.api_resources.payment_link import PaymentLink
    from xpay.api_resources.setup_intent import SetupIntent
    from xpay.api_resources.shipping_rate import ShippingRate
    from xpay.api_resources.subscription import Subscription
    from xpay.api_resources.tax_id import TaxId as TaxIdResource
...
class Session(
    ...
    one-time purchases or subscriptions through [Checkout](https://xpay.com/docs/payments/checkout)
    or [Payment Links](https://xpay.com/docs/payments/payment-links). We recommend creating a
    new Session each time your customer attempts to pay.

    Once payment is successful, the Checkout Session will contain a reference
    to the [Customer](https://xpay.com/docs/api/customers), and either the successful
    [PaymentIntent](https://xpay.com/docs/api/payment_intents) or an active
    [Subscription](https://xpay.com/docs/api/subscriptions).

    You can create a Checkout Session on your server and redirect to its URL
    to begin Checkout.

    Related guide: [Checkout quickstart](https://xpay.com/docs/checkout/quickstart)
    """

    OBJECT_NAME: ClassVar[Literal["checkout.session"]] = "checkout.session"

    class AfterExpiration(XPayObject):
        class Recovery(XPayObject):
    ...
    class ConsentCollection(XPayObject):
    ...
    class CurrencyConversion(XPayObject):
        amount_subtotal: int
        """
    ...
    class CustomField(XPayObject):
        class Dropdown(XPayObject):
            class Option(XPayObject):
                label: str
                """
    ...
    class CustomText(XPayObject):
        class ShippingAddress(XPayObject):
            message: str
            """
            Text may be up to 1200 characters in length.
            """

        class Submit(XPayObject):
            message: str
            """
            Text may be up to 1200 characters in length.
            """

        class TermsOfServiceAcceptance(XPayObject):
            message: str
            """
            Text may be up to 1200 characters in length.
            """
    ...
    class CustomerDetails(XPayObject):
    ...
    class InvoiceCreation(XPayObject):
        class InvoiceData(XPayObject):
            class CustomField(XPayObject):
                name: str
                """
            ...
            class RenderingOptions(XPayObject):
    ...
    class PaymentMethodOptions(XPayObject):
        class AcssDebit(XPayObject):
            class MandateOptions(XPayObject):
                ...
                """
                List of XPay products where this mandate can be selected automatically. Returned when the Session is in `setup` mode.
                """
    ...
    class PhoneNumberCollection(XPayObject):
        enabled: bool
        """
        Indicates whether phone number collection is enabled for the session
        """

    class ShippingAddressCollection(XPayObject):
    ...
    class ShippingDetails(XPayObject):
        class Address(XPayObject):
            city: Optional[str]
            """
            City, district, suburb, town, or village.
    ...
    class ShippingOption(XPayObject):
        shipping_amount: int
        """
    ...
    class TaxIdCollection(XPayObject):
        enabled: bool
        """
        Indicates whether tax ID collection is enabled for the session
        """

    class TotalDetails(XPayObject):
        class Breakdown(XPayObject):
    ...
    if TYPE_CHECKING:
        ...
        class CreateParams(RequestOptions):
            after_expiration: NotRequired[
                "Session.CreateParamsAfterExpiration"
            ]
            """
            ...
            """
            Three-letter [ISO currency code](https://www.iso.org/iso-4217-currency-codes.html), in lowercase. Must be a [supported currency](https://xpay.com/docs/currencies). Required in `setup` mode when `payment_method_types` is not set.
            """
            ...
            on the Checkout page. In `subscription` mode, the customer's [default payment method](https://xpay.com/docs/api/customers/update#update_customer-invoice_settings-default_payment_method)
            will be used if it's a card, otherwise the most recently saved card will be used. A valid billing address, billing name and billing email are required on the payment method for Checkout to prefill the customer's card details.

            If the Customer already has a valid [email](https://xpay.com/docs/api/customers/object#customer_object-email) set, the email will be prefilled and not editable in Checkout.
            If the Customer does not have a valid `email`, Checkout will set the email entered during the session on the Customer.

            If blank for Checkout Sessions in `subscription` mode or with `customer_creation` set as `always` in `payment` mode, Checkout will create a new Customer object based on information provided during the payment flow.

            You can set [`payment_intent_data.setup_future_usage`](https://xpay.com/docs/api/checkout/sessions/create#create_checkout_session-payment_intent_data-setup_future_usage) to have Checkout automatically attach the payment method to the Customer you pass in for future reuse.
            """
            customer_creation: NotRequired["Literal['always', 'if_required']"]
            """
            Configure whether a Checkout Session creates a [Customer](https://xpay.com/docs/api/customers) during Session confirmation.

            When a Customer is not created, you can still retrieve email, address, and other customer data entered in Checkout
            with [customer_details](https://xpay.com/docs/api/checkout/sessions/object#checkout_session_object-customer_details).

            Sessions that don't create Customers instead are grouped by [guest customers](https://xpay.com/docs/payments/checkout/guest-customers)
            ...
            invoice_creation: NotRequired[
                "Session.CreateParamsInvoiceCreation"
            ]
            """
            ...
            """
            A list of items the customer is purchasing. Use this parameter to pass one-time or recurring [Prices](https://xpay.com/docs/api/prices).
            ...
            If you'd like information on how to collect a payment method outside of Checkout, read the guide on configuring [subscriptions with a free trial](https://xpay.com/docs/payments/checkout/free-trials).
            """
            ...
            You can omit this attribute to manage your payment methods from the [XPay Dashboard](https://dashboard.xpay.com/settings/payment_methods).
            See [Dynamic Payment Methods](https://xpay.com/docs/payments/payment-methods/integration-options#using-dynamic-payment-methods) for more details.

            Read more about the supported payment methods and their requirements in our [payment
            method details guide](https://xpay.com/docs/payments/checkout/payment-methods).
            ...
            before using this feature. Learn more about [collecting phone numbers with Checkout](https://xpay.com/docs/payments/checkout/phone-numbers).
            """
            redirect_on_completion: NotRequired[
                "Literal['always', 'if_required', 'never']"
            ]
            """
            This parameter applies to `ui_mode: embedded`. By default, XPay will always redirect to your return_url after a successful confirmation. If you set `redirect_on_completion: 'if_required'`, then we will only redirect if your user chooses a redirect-based payment method.
            """
            ...
            setup_intent_data: NotRequired[
                "Session.CreateParamsSetupIntentData"
            ]
            """
            ...
            """
            The URL to which XPay should send customers when payment or setup
            is complete.
            This parameter is not allowed if ui_mode is `embedded`. If you'd like to use
            information from the successful Checkout Session on your page, read the
            guide on [customizing your success page](https://xpay.com/docs/payments/checkout/custom-success-page).
            """
            tax_id_collection: NotRequired[
                "Session.CreateParamsTaxIdCollection"
            ]
            """
        ...
        class CreateParamsSubscriptionData(TypedDict):
            ...
            """
            A non-negative decimal between 0 and 100, with at most two decimal places. This represents the percentage of the subscription invoice total that will be transferred to the application owner's XPay account. To use an application fee percent, the request must be made on behalf of another account, using the `XPay-Account` header or an OAuth key. For more information, see the application fees [documentation](https://xpay.com/docs/connect/subscriptions#collecting-fees-on-subscriptions).
            """
            ...
            for rendering in the [customer portal](https://xpay.com/docs/customer-management).
            """
            metadata: NotRequired["Dict[str, str]"]
            """
            Set of [key-value pairs](https://xpay.com/docs/api/metadata) that you can attach to an object. This can be useful for storing additional information about the object in a structured format. Individual keys can be unset by posting an empty value to them. All keys can be unset by posting an empty value to `metadata`.
            ...
            proration_behavior: NotRequired[
                "Literal['create_prorations', 'none']"
            ]
            """
        ...
        class CreateParamsShippingOptionShippingRateData(TypedDict):
            ...
            """
            A [tax code](https://xpay.com/docs/tax/tax-categories) ID. The Shipping tax code is `txcd_92010001`.
            """
        ...
        class CreateParamsShippingOptionShippingRateDataFixedAmount(TypedDict):
            ...
            """
            Shipping rates defined in each available currency option. Each key must be a three-letter [ISO currency code](https://www.iso.org/iso-4217-currency-codes.html) and a [supported currency](https://xpay.com/docs/currencies).
            """
        ...
        class CreateParamsSetupIntentData(TypedDict):
            ...
            on_behalf_of: NotRequired["str"]
            """
            The XPay account for which the setup is intended.
            """
        ...
        class CreateParamsPaymentMethodOptions(TypedDict):
            ...
            """
            affirm: NotRequired[
                "Session.CreateParamsPaymentMethodOptionsAffirm"
            ]
            """
            ...
            """
            alipay: NotRequired[
                "Session.CreateParamsPaymentMethodOptionsAlipay"
            ]
            """
            ...
            """
            boleto: NotRequired[
                "Session.CreateParamsPaymentMethodOptionsBoleto"
            ]
            """
            ...
            """
            cashapp: NotRequired[
                "Session.CreateParamsPaymentMethodOptionsCashapp"
            ]
            """
            ...
            """
            giropay: NotRequired[
                "Session.CreateParamsPaymentMethodOptionsGiropay"
            ]
            """
            contains details about the Giropay payment method options.
            """
            grabpay: NotRequired[
                "Session.CreateParamsPaymentMethodOptionsGrabpay"
            ]
            """
            ...
            """
            konbini: NotRequired[
                "Session.CreateParamsPaymentMethodOptionsKonbini"
            ]
            """
            ...
            """
            paynow: NotRequired[
                "Session.CreateParamsPaymentMethodOptionsPaynow"
            ]
            """
            contains details about the PayNow payment method options.
            """
            paypal: NotRequired[
                "Session.CreateParamsPaymentMethodOptionsPaypal"
            ]
            """
            ...
            """
            sofort: NotRequired[
                "Session.CreateParamsPaymentMethodOptionsSofort"
            ]
            """
        ...
        class CreateParamsPaymentMethodOptionsAcssDebit(TypedDict):
            ...
            """
            Three-letter [ISO currency code](https://www.iso.org/iso-4217-currency-codes.html), in lowercase. Must be a [supported currency](https://xpay.com/docs/currencies). This is only accepted for Checkout Sessions in `setup` mode.
            """
        ...
        class CreateParamsPaymentMethodOptionsAcssDebitMandateOptions(
            ...
            default_for: NotRequired[
                "List[Literal['invoice', 'subscription']]"
            ]
            """
            List of XPay products where this mandate can be selected automatically. Only usable in `setup` mode.
            """
        ...
        class CreateParamsPaymentIntentData(TypedDict):
            ...
            on_behalf_of: NotRequired["str"]
            """
            The XPay account ID for which these funds are intended. For details,
            see the PaymentIntents [use case for connected
            accounts](https://xpay.com/docs/payments/connected-accounts).
            """
            ...
            """
            Indicates that you intend to [make future payments](https://xpay.com/docs/payments/payment-intents#future-usage) with the payment
            ...
            shipping: NotRequired[
                "Session.CreateParamsPaymentIntentDataShipping"
            ]
            """
            ...
            For more information, see the PaymentIntents [use case for connected accounts](https://xpay.com/docs/payments/connected-accounts).
            """
            transfer_group: NotRequired["str"]
            """
            A string that identifies the resulting payment as part of a group. See the PaymentIntents [use case for connected accounts](https://xpay.com/docs/connect/separate-charges-and-transfers) for details.
            """
        ...
        class CreateParamsLineItem(TypedDict):
            ...
            """
            The [tax rates](https://xpay.com/docs/api/tax_rates) that will be applied to this line item depending on the customer's billing/shipping address. We currently support the following countries: US, GB, AU, and all countries in the EU.
            """
            price: NotRequired["str"]
            """
            The ID of the [Price](https://xpay.com/docs/api/prices) or [Plan](https://xpay.com/docs/api/plans) object. One of `price` or `price_data` is required.
            """
            price_data: NotRequired["Session.CreateParamsLineItemPriceData"]
            """
            Data used to generate a new [Price](https://xpay.com/docs/api/prices) object inline. One of `price` or `price_data` is required.
            """
            ...
            """
            The [tax rates](https://xpay.com/docs/api/tax_rates) which apply to this line item.
            """
        ...
        class CreateParamsLineItemPriceDataProductData(TypedDict):
            ...
            tax_code: NotRequired["str"]
            """
            A [tax code](https://xpay.com/docs/tax/tax-categories) ID.
            """
        ...
        class CreateParamsCustomText(TypedDict):
            ...
            """
            submit: NotRequired[
                "Literal['']|Session.CreateParamsCustomTextSubmit"
            ]
            """
            Custom text that should be displayed alongside the payment confirmation button.
            """
        ...
        class CreateParamsConsentCollection(TypedDict):
            ...
            There must be a valid terms of service URL set in your [Dashboard settings](https://dashboard.xpay.com/settings/public).
            """
        ...
        class CreateParamsAfterExpiration(TypedDict):
            recovery: NotRequired[
                "Session.CreateParamsAfterExpirationRecovery"
            ]
            """
    ...
    """
    Client secret to be used when initializing XPay.js embedded checkout.
    """
    ...
    """
    Applies to Checkout Sessions with `ui_mode: embedded`. By default, XPay will always redirect to your return_url after a successful confirmation. If you set `redirect_on_completion: 'if_required'`, then we will only redirect if your user chooses a redirect-based payment method.
    """
    return_url: Optional[str]
    """
    ...
    url: Optional[str]
    """
    The URL to the Checkout Session. Redirect customers to this URL to take them to Checkout. If you're using [Custom Domains](https://xpay.com/docs/payments/checkout/custom-domains), the URL will use your subdomain. Otherwise, it'll use `checkout.xpay.com.`
