...
class Card(DeletableAPIResource["Card"], UpdateableAPIResource["Card"]):
    ...
    """
    Three-letter [ISO code for currency](https://xpay.com/docs/payouts). Only applicable on accounts (not customers or recipients). The card can be used as a transfer destination for funds in this currency.
    """
    ...
    *As of May 1, 2021, card fingerprint in India for Connect changed to allow two fingerprints for the same card---one for India and one for the rest of the world.*
    ...
    status: Optional[str]
    """
    For external accounts that are cards, possible values are `new` and `errored`. If a payout fails, the status is set to `errored` and [scheduled payouts](https://xpay.com/docs/payouts#payout-schedule) are stopped until account details are updated.
    """
    ...
    def modify(cls, sid, **params):
        raise NotImplementedError(
            ...
            "Use xpay.Customer.modify_source('customer_id', 'card_id', ...) "
            "(see https://xpay.com/docs/api/cards/update) or "
            "xpay.Account.modify_external_account('account_id', 'card_id', ...) "
            "(see https://xpay.com/docs/api/external_account_cards/update)."
        )
    ...
    def retrieve(
        ...
        **params
        ...
        raise NotImplementedError(
            ...
            "Use xpay.Customer.retrieve_source('customer_id', 'card_id') "
            "(see https://xpay.com/docs/api/cards/retrieve) or "
            "xpay.Account.retrieve_external_account('account_id', 'card_id') "
            "(see https://xpay.com/docs/api/external_account_cards/retrieve)."
        )
