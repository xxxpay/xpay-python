...
class Token(ListableAPIResource["Token"], UpdateableAPIResource["Token"]):
    """
    An issuing token object is created when an issued card is added to a digital wallet. As a [card issuer](https://xpay.com/docs/issuing), you can [view and manage these tokens](https://xpay.com/docs/issuing/controls/token-management) through XPay.
    """

    OBJECT_NAME: ClassVar[Literal["issuing.token"]] = "issuing.token"

    class NetworkData(XPayObject):
        class Device(XPayObject):
        ...
        class WalletProvider(XPayObject):
            class CardholderAddress(XPayObject):
                line1: str
                """
